...
<div class="card-custom">
    <div class="text-center mb-4">
        ...
        <h1 class="display-5 fw-bold text-primary mt-2">{{ $submenu_nombre }}</h1>
        <p class="lead text-secondary">Registro semanal de actividades</p>
        <p class="text-muted">Semana #{{ date('W') }} - Año {{ date('Y') }}</p>
    </div>
                        ...
                        <table class="table table-bordered table-hover">
                                ...
                                @foreach($actividadesUnidad as $index => $actividad)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <input type="hidden" name="actividades[{{ $actividad->id_actividades }}][id]" value="{{ $actividad->id_actividades }}">
                                            <input type="hidden" name="actividades[{{ $actividad->id_actividades }}][nombre]" value="{{ $actividad->actividades }}">
                                        </td>
                                        <td>
                                            <input type="number" 
                                                   name="actividades[{{ $actividad->id_actividades }}][cantidad]" 
                            ...
                            <tfoot class="table-light">
                                <tr>
                                    ...
                                    <td class="text-center fw-bold text-primary total-unidad" id="total-{{ Str::slug($unidad) }}">0</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
